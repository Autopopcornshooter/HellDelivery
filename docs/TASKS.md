...
- [x] 1~4인 좌석/운전/제동/하차와 호스트 권위 동기화, 입력 지연 제동.
- [x] 독립 물리 화물, 실제 적재/주차/주소 판정, 직접 하역→기존 계단 배송.
- [x] 충격 손상·파손/분실·시간 초과/차량 운행 불가·S/A/B/C/F·점수·복구 감점.
...
사용자가 진행 현황 1·2단계를 한 묶음으로 승인했다. 네 자리 로비/캐릭터/전원 준비, 슬롯별 입력과 복구, 4인 공동 잡기, 참가자별 지연 정지, 도움 요청/전원 종료 동의, 3인 전환과 재참가, 기존 주문/3연속 코스를 통합했다. 계단 해제는 접촉 침투와 가상 운반 목표점의 벽 통과를 재현하여 실제 시점→택배 중심 차단 경로로 수정했다. 검증 및 전달 기록은 WINDOWS_PLAYTEST_47.md. 실제 여러 PC에서 코스 전체 수동 운반/외부망 확인은 남으며, 이 확인 이후 다음 큰 묶음은 차량과 전체 배송 루프다. 차량·경제·Steam·push는 이번 범위 아님.
