...
| 항목 | 내용 |
|---|---|
| **Current Status** | **v0.3.0 Complete — v0.4.0(Steam Demo) In Progress (EPIC-07, T083 `[BLOCKED]` Export Template 설치 대기)** |
| **Current Version** | v0.3.0 — Fun Physics Update Complete (v0.4.0은 아직 미완료, 데모 구현 진행 중) |
| **Development Stage** | v0.2.0·v0.3.0 모두 완료. v0.4.0(Steam Demo) 구현 진행 중 — T077~T082 `[DONE]`(전부 사용자 승인 완료), T083(Windows Export and Build Validation) `[BLOCKED]`(Preset 구성·검증 완료, Export Template 미설치로 실제 빌드 검증 보류) |
| **Current Phase** | Gameplay Expansion 완료, Steam Demo 구현 중 (`docs/TASKS.md` T064~T082 전부 `[DONE]`, T083 `[BLOCKED]`; EPIC-01~06 전부 완료, EPIC-07 진행 중 — FEATURE-07-A/B/C/D 완료, FEATURE-07-E(T083) Preset 구성 완료·Export Template 대기) |
| **Current Milestone** | Milestone 2 완료. Milestone 3(Steam Demo) 구현 진행 중(`MILESTONES.md` 참고) |
| **Engine** | Godot 4.7.1 (stable, win64) |
| **Language** | GDScript |
| **Physics Engine** | Jolt Physics |
| **Renderer** | Forward+ (Windows: D3D12) |
| **Repository State** | 정상 동작하는 기준선(Baseline) — `docs/TASKS.md` T000~T082 전체 완료, T083(Windows Export and Build Validation) `[BLOCKED]`(Preset 구성·검증 완료, Export Template 미설치) |
...
| **Next Milestone** | Milestone 3 — Steam Demo(`MILESTONES.md`, `ROADMAP.md` v0.4.0 참고) — T083(Windows Export)은 Export Template 설치 후 재개, 이어서 T084(성능)·T085(최종 플레이테스트) 순으로 진행 예정 |
...
- **완료(v0.4.0 진행분)**: v0.4.0(Steam Demo) EPIC-07(Steam Demo Readiness) 중 FEATURE-07-A/B/C/D 완료. T077(Demo Readiness Audit & Scope Lock, `[DONE]`) — 조사·계획만 수행, 데모 범위 안 A(싱글 중심) 확정, 후속 Task(T078~T085) 계획. T078(Main Menu & Demo Entry, `[DONE]`) — `MainMenu.tscn`을 `run/main_scene`으로 지정, 데모 시작·게임 종료. T079(Pause, Settings & Exit Flow, `[DONE]`) — `PauseMenu`·`SettingsPanel`·`GameSettings` Autoload(설정 저장/복원). T080(Player Onboarding & Controls, `[DONE]`) — 첫 실행 안내 Overlay, 공용 조작법 화면. T081(Demo Gameplay Loop & Completion Flow, `[DONE]`) — Package 3개 반복 배송, 진행도/토스트 HUD, 플레이 타이머, 완료 화면(다시 플레이/메인 메뉴). T082(Audio and Feedback Pass, `[DONE]`) — 절차적 효과음 8개(외부 음원 없이 직접 합성)와 `AudioManager` Autoload, DeliveryZone 시각 Pulse·배송 Toast·완료 화면 진입 연출. 6개 Task 전부 헤드리스 자동 검증과 사용자 수동 테스트 승인을 모두 거쳤다.
- **진행 중(BLOCKED, Export Template 설치 대기)**: v0.4.0 EPIC-07 FEATURE-07-E — T083(Windows Export and Build Validation). `export_presets.cfg`(Windows Desktop Release Preset)를 신규 구성해 설정 오류 없이 검증했으나, 로컬에 Godot 4.7.1.stable용 Export Template이 설치되어 있지 않아 실제 `.exe` 생성·실행 검증은 진행하지 못했다(임의 다운로드 없이 정확한 설치 방법만 `docs/TASKS.md` 섹션 35에 보고).
