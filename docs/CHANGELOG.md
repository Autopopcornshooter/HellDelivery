...
v0.4.0 범위의 구현 작업 착수. `docs/TASKS.md` T077(Demo Readiness Audit & Scope Lock) `[DONE]` — 조사 전용 Task로 코드 변경은 없으며, 사용자가 권장안(안 A — 싱글플레이 중심 데모)을 승인해 v0.4.0 범위를 확정했다. T078(Main Menu & Demo Entry) `[DONE]`(사용자 승인) — 실행 시 곧바로 테스트 레벨로 들어가던 것을 멈추고, 제목·데모 시작·게임 종료만 갖춘 최소 메인 메뉴를 먼저 보여주도록 했으며, v0.4.0 데모 Baseline으로 기록되었다. T079(Pause, Settings & Exit Flow) `[DONE]`(사용자 승인) — Esc 일시정지, MainMenu·PauseMenu 공용 설정 화면(화면/입력/오디오), `user://settings.cfg` 저장·복원을 추가했으며, Pause/Settings/Exit 흐름과 현재 설정값이 v0.4.0 Baseline으로 기록되었다. T080(Player Onboarding & Controls) `[DONE]`(사용자 승인) — 첫 실행 안내 Overlay, MainMenu·PauseMenu 공용 조작법 화면, 짧은 목표 문구를 추가했으며, 온보딩·조작법 UI가 v0.4.0 Baseline으로 기록되었다. 이어서 T081(Demo Gameplay Loop & Completion Flow) `[DONE]`(사용자 승인) — 단일 배송 판정을 Package 3개 반복 배송으로 확장하고, 진행도/토스트 HUD, 플레이 타이머, 완료 화면(다시 플레이/메인 메뉴)을 추가했으며, 완결된 데모 플레이 흐름이 v0.4.0 Baseline으로 기록되었다. 이어서 T082(Audio and Feedback Pass) `[DONE]`(사용자 승인) — 무음이었던 데모에 절차적 효과음 8개(외부 음원 없음)와 `AudioManager` Autoload, DeliveryZone 시각 Pulse, 배송 Toast/완료 화면 진입 연출을 추가했으며, 자동 검증(82개 항목 3회 연속 PASS)과 사용자 수동 테스트 승인을 모두 거쳤다. 이어서 T083(Windows Export and Build Validation) `[DONE]`(사용자 승인) — `export_presets.cfg`(Windows Desktop Release Preset)를 신규 구성하고 Export Template 설치 후 Release Export에 성공했으며, 사용자가 생성된 `HellDelivery.exe`(저장소 밖 `D:\Program Files\GitHub\HellDeliveryBuild\`)를 직접 실행해 MainMenu·데모 시작·배송·완료 화면·Pause/Restart/설정/오디오·정상 종료를 모두 승인했다. 이어서 T084(Performance & Compatibility Pass) `[DONE]`(사용자 승인) — Release 빌드의 성능(구간별 프레임 시간 프록시)·해상도/입력 호환성(코드 검토)·안정성(EXE 10회 시작·종료, Restart/메뉴 왕복 10회)을 점검했으며, 명확한 실행 차단 결함이 없어 코드는 수정하지 않았고 사용자가 실제 EXE로 직접 확인해 승인했다. 이어서 사용자가 T085를 T085A(Art Direction & Delivery Map Blockout)/T085B(Environment Art & Lighting Pass)/T085C(Level Polish & Playability) 세 단계로 세분화했다(기존 Final Demo Playtest 검증 내용은 T086 `[TODO]`로 이동, 새 맵 기준 재수행 예정). T085A `[DONE]`(사용자 승인) — 테스트 공간처럼 보이던 `PrototypeLevel`을 택배 트럭 출발(A) → 초반 기본 경로(B) → 물리 오브젝트 장애물 극복 구간(C) → 계단+좁은 문 결합 구간(D) → 건물 배송 구역(E)으로 이어지는, 공간적으로 구분된 5구역 데모 맵으로 확장했다(맵 크기 20×20→20×30, 신규 `scenes/environment/DeliveryTruck.tscn`, 건물 파사드, 장애물 크레이트 구간, 미니 계단, 구역 표지판, Player·Package·DeliveryZone·PhysicsObjects Transform 재배치 — Grab·Delivery 로직과 기존 계단·경사로·좁은 문의 물리값·Transform은 무변경). 외부 Asset 사용 정책과 아트 방향성(구역별 색 규칙)을 `docs/ASSET_LICENSES.md`에 신규 문서화했으며, 자동 검증(34개 항목 3회 연속 PASS)·Release Export 재확인·사용자 수동 테스트 승인을 모두 거쳤다. 이어서 T085B(Environment Art, Lighting, Visual Target Pass) `[REVIEW]` — T085A의 블록아웃 구조·동선은 그대로 유지한 채, 조명(웜톤 지향광·그림자·안개·SSAO·약한 Glow), 트럭 Hero Pass(전면 유리·전조등·후미등·범퍼·사이드미러·로고·적재함 내부 조명), Package 시각 개선(카드보드 재질·테이프·라벨·상하/취급주의 표시), 장애물 구간 경고 스트라이프, 방향 화살표, 배경 실루엣·펜스를 추가해 맵의 완성도와 인상을 끌어올렸다(전부 기본 Godot Primitive만 사용, 외부 Asset은 이번에도 도입하지 않음). 자동 검증(49개 항목 3회 연속 PASS)과 Release Export 재확인을 마쳤다. 이어서 사용자가 T084 재승인 후 T085A를 세 번째로 재정의했다 — "Delivery Demo Level Design & Greybox": `PrototypeLevel.tscn`(물리 회귀 테스트 전용으로 보존, 공개 메뉴 미노출)을 대체할 완전히 새로운 독립 Scene `scenes/level/DeliveryDemoLevel.tscn`을 신규 설계·구현했다. "언덕 아래 배송 트럭 → 서비스 골목 → 경사 하역장 → 공사 중 안뜰 → 좁은 후문·계단 → 배송 로비"로 이어지는 6구역(A~F) Greybox이며, 설계 문서 `docs/LEVEL_DESIGN_DEMO.md`를 Scene 작업 전에 먼저 작성했다. 강제 L자 병목(폭 1.3m), 약 12° 경사 하역장(회복 구간 포함), HingeJoint3D 기반 자동 닫힘 게이트(`scenes/environment/DemoGate.tscn`, 물체로 버팀 가능), 크레이트로 만드는 임시 발판(낮은 턱 구간), 계단+좁은 문 결합 최종 접근 구간, `DeliveryZone`으로의 지름길 복귀 경로를 구현했다. Package는 크기·질량만 다른 3종(`Package.tscn` 무변경/`PackageLong.tscn`/`PackageHeavy.tscn` 신규, 동일 경로·동일 목적지)으로 차별화했으며, `PrototypeLevel.gd`를 그대로 재사용해 신규 스크립트 코드 없이 기존 Player/HUD/Delivery/Pause/온보딩 로직을 재사용했다. 구현 중 경사면 회전 부호 오류(자유낙하 유발)·게이트 문짝-기둥 충돌 겹침(경첩 고착)·게이트 스윙 경로와 벽 충돌 3건의 실제 버그를 발견해 수정했다. `MainMenu.gd`의 데모 진입 경로를 새 레벨로 변경했다(`PrototypeLevel.tscn`은 에디터 F6 회귀 테스트용으로 계속 보존, `LocalCoopTest.tscn`도 삭제하지 않음). 자동 검증(39개 항목 3회 연속 PASS)과 Release Export 재확인을 마쳤으며, 사용자 수동 테스트 대기 중이라 v0.4.0은 아직 완료 처리하지 않는다. T085B(Environment Art & Lighting)·T085C(Level Polish & Balance)는 새 레벨 기준으로 `[TODO]` 재설정되었다.
...
- `assets/audio/generated/`에 절차적 효과음 8개(WAV) 신규 — 외부 음원 없이 순수 사인파/화이트노이즈 합성(`ui_select`/`ui_back`/`ui_pause_open`/`grab_success`/`grab_release`/`grab_forced_release`/`delivery_success`/`delivery_complete`, T082)
- `autoload/AudioManager.gd` 신규(이 프로젝트 두 번째 Autoload) — `play_ui()`/`play_2d()`/`play_3d()` 세 API만 제공하는 최소 효과음 재생 관리자. 카테고리별 고정 크기 `AudioStreamPlayer`/`AudioStreamPlayer3D` 풀(UI 4개, 2D 4개, 3D 8개)을 재사용해 무제한 생성을 막고, 존재하지 않는 Stream 요청은 조용히 무시한다. 전부 기본 `"Master"` Bus를 사용해 `GameSettings`의 Master Volume이 추가 배선 없이 그대로 적용된다(T082)
- `Player.gd`에 Grab 성공/수동 Release/강제 해제(거리 초과·차단) 3D 위치 기반 효과음 추가 — 기존 `add_grabber()` 성공 분기와 T075 `grabber_disconnected`/`_on_held_grabbable_disconnected` 콜백을 그대로 재사용해 사유별로 다른 효과음을 재생한다(T082)
- `DeliveryHUD.gd`의 `show_delivery_toast()`에 배송 성공 2D 효과음과 Toast 확대·복귀 Tween(0.12초 왕복) 추가(T082)
- `PrototypeLevel.gd`의 `_on_all_packages_delivered()`에 최종 완료 2D 효과음 추가, `CompletionOverlay.gd`에 Fade+Scale 진입 연출(0.35초) 추가 — `CanvasLayer`가 `PROCESS_MODE_ALWAYS`라 `paused=true` 이후에도 연출이 계속되며 버튼 입력을 지연시키지 않는다(T082)
- `DeliveryZone.gd`의 `MeshInstance3D`에 발광 재질을 부여하고 2.4초 주기로 밝기를 오가는 느린 Pulse 추가 — `CollisionShape3D`/판정 로직은 무변경(T082)
- `MainMenu.gd`/`PauseMenu.gd`/`SettingsPanel.gd`/`ControlsPanel.gd`/`CompletionOverlay.gd`에 버튼 선택·뒤로 가기·Pause 열기 UI 효과음 추가(T082)
- `export_presets.cfg` 신규(이 프로젝트 최초의 Export Preset) — Windows Desktop / x86_64 / Release Export Console Wrapper 미포함(일반 사용자에게 콘솔 창 비노출) / `application/export_d3d12=true`(`project.godot`의 `rendering_device/driver.windows="d3d12"` 설정과 일치) / 코드 서명·Steamworks 연동 없음 / 빌드 결과물 경로는 저장소 밖(`d:\Program Files\GitHub\HellDeliveryBuild\`)(T083)
- `scenes/environment/DeliveryTruck.tscn` 신규(이 프로젝트 최초의 `environment` 하위 재사용 Scene) — 운전 불가능한 정적 `StaticBody3D` 택배 트럭. 운전석·적재함 바닥/좌우벽/지붕 5개 Collision과 순수 시각 전용 바퀴 4개로 구성, 적재함 뒤쪽(남쪽)은 벽 없이 열려 있어 Player가 Package를 꺼낼 수 있다. 차량 물리·문 애니메이션·운전 기능은 없다(T085A)
- `PrototypeLevel.tscn`에 `DeliveryTruck` 인스턴스, `DeliveryBuilding`(`BackWall`/`WestWall`/`EastWall`/`Canopy`/`EntranceMat`/`EntranceSign`으로 구성된 순수 시각적 건물 파사드), 바닥 표시(`ParkingPad`)와 안내 표지판(`Label3D`) 추가 — 전부 물리 동선을 막지 않는 장식(건물 벽 3면은 Collision 있음, 캐노피·바닥 표시·표지판은 Collision 없음)(T085A)
- `docs/ASSET_LICENSES.md` 신규 — 외부 Asset 사용 정책(직접 제작 Blender 모델·CC0/명확한 무료 라이선스만 허용, 스타일 통일 필수)과 구역별 아트 방향성(색 규칙)을 문서화. 이번 단계에서 실제 외부 Asset은 도입하지 않았다(T085A)
- `PrototypeLevel.tscn`의 `Floor`를 20×1×20→20×1×30으로 확장해 남쪽 공간을 확보하고, `HazardStackLeft`/`HazardStackRight`(정적 장애물 한 쌍, 노란 계열 강조색)와 그 사이에 놓인 `PhysicsObjects/MovableCrate`(기존 `PhysicsCrate.tscn` 재사용 인스턴스)로 "밀어서 통과하면 훨씬 수월해지는" 물리 오브젝트 장애물 구간을 신규 추가 — 새 물리 시스템 없이 기존 Grab/Push만 사용(T085A)
- `MiniStairs`(4단, 기존 `Stairs`의 Step Shape를 90° 회전 재사용) 신규 추가 — 좁은 문(기존 `NarrowDoorwayTestArea`)과 결합된 구간을 구성. `Ramp`/`Stairs`(원본) 등 기존 구조물은 Transform을 포함해 전혀 변경하지 않았다(T085A)
- 구역 전환 안내 `Label3D` 4종("배송 출발"/"주의: 좁은 통로"/"장애물: 밀어서 치우기"/"계단 주의") 추가, 기존 "배송 접수처"와 함께 5구역(트럭 출발/초반 경로/물리 장애물/계단+좁은 문/건물) 전환을 안내(T085A)
- `DeliveryTruck.tscn`에 Hero Pass 10종(전면 유리·전조등 2·후미등 2·범퍼·사이드미러 2·로고·적재함 내부 `OmniLight3D`) 추가 — 전부 `MeshInstance3D`(또는 `OmniLight3D`)만 추가해 신규 Collision 없음, 운전 기능·복잡한 애니메이션 없음(T085B)
- `Package.tscn`에 카드보드색 기본 재질(이전에는 무재질 기본 회색)과 시각 요소 4종(테이프 띠, 라벨 패널, "▲ TOP" 회전 고정 라벨, 붉은 "FRAGILE" 라벨) 추가 — `CollisionShape3D`·`mass`·`PhysicsMaterial`·감쇠값·`Package.gd`는 전혀 변경하지 않았다(T085B)
- `PrototypeLevel.tscn`에 조명·환경 보강: `DirectionalLight3D` 각도(X축 -50°)·웜톤 색·그림자, `WorldEnvironment`에 웜톤 Ambient·옅은 Fog·Filmic Tonemap·약한 Glow·약한 SSAO, `ProceduralSkyMaterial`에 노을 톤 하늘/지평선 색 추가(T085B)
- `DeliveryBuilding`에 청록색 `AccentLight`(`OmniLight3D`), `HazardStackLeft`/`Right`에 검정 `WarningStripe`, 트럭 출발·문 진입 지점에 노란 방향 화살표 `Label3D` 2종, 맵 네 모서리에 원거리 실루엣 4개·펜스 2개(전부 Collision 없음) 추가(T085B)
- `docs/LEVEL_DESIGN_DEMO.md` 신규 — `DeliveryDemoLevel.tscn` Scene 작업 전에 먼저 작성한 설계 문서. 6구역(A~F) 구조 다이어그램, Critical Path, 구역별 역할, Package 3종 배송 비트, 솔로/협동 해법, 물리 오브젝트 목록, 병목·경사·턱 치수 근거, 예상 플레이 시간, 소프트락 방지 원칙, T085B 에셋 우선순위 목록을 포함(T085A 재정의)
- `scenes/level/DeliveryDemoLevel.tscn` 신규 — `PrototypeLevel.gd`를 그대로 재사용하는 완전히 새로운 6구역(A. 트럭 마당/B. 서비스 골목/C. 경사 하역장/D. 공사 중 안뜰/E. 최종 접근/F. 배송 로비) Greybox 데모 레벨. 강제 L자 병목(폭 1.3m), 회복 구간 있는 12° 경사, 자동 닫힘 게이트, 임시 발판용 크레이트 턱, 계단+좁은 문 결합 구간, `DeliveryZone`으로의 지름길 복귀 경로 포함(T085A 재정의)
- `scenes/package/PackageLong.tscn`/`PackageHeavy.tscn` 신규 — `Package.gd`를 그대로 재사용하되 형태·질량만 다른 Package 변형(각각 1.4×0.5×0.6/질량18, 0.9×0.8×0.9/질량32). `Package.tscn`(0.8×0.6×0.8/질량15)은 무변경(T085A 재정의)
- `scenes/environment/DemoGate.tscn` 신규 — `HingeJoint3D` 기반 자동 닫힘 게이트. 힘을 받으면 열리고 Motor로 서서히 자동 복귀하며, 근처 물체(`GateProp`)로 열린 상태를 버틸 수 있다(T085A 재정의)
...
- `PrototypeLevel.tscn`의 `Player`/`Package`/`PackageB`/`PackageC`/`DeliveryZone`/`PhysicsObjects`(신규 `MovableCrate` 포함 7종) Transform을 트럭 출발(A)→초반 경로(B)→물리 장애물 구간(C)→계단+좁은 문 결합 구간(D)→건물 배송 구역(E) 5구역 단일 동선에 맞게 재배치 — 각 씬의 물리값·Collision Shape·스크립트는 무변경, 기존 `Stairs`/`Ramp`/`WallTestArea`/`NarrowDoorwayTestArea`의 Transform도 전혀 건드리지 않았다. `DeliveryBuilding`/`DeliveryZone`은 확장된 맵 공간을 위해 3m 추가 이동했다(T085A)
- `MainMenu.gd`의 `DEMO_SCENE_PATH`를 `PrototypeLevel.tscn`에서 `DeliveryDemoLevel.tscn`으로 변경 — `PrototypeLevel.tscn`은 물리 회귀 테스트 전용으로 보존되며 에디터 F6으로 계속 실행 가능하다(공개 메뉴에는 노출하지 않음), `LocalCoopTest.tscn`도 삭제하지 않고 계속 F6 전용으로 유지한다(T085A 재정의)
...
- T082의 효과음 8개(주파수·길이·음량)는 실측 후보 비교가 아닌 초기 추정값 — 사용자 수동 테스트에서 승인되었으나 초기 추정값이라는 점 자체는 `TECH_DEBT.md` TD-022로 계속 추적
- Windows Export(T083) — `.exe` 생성·실행부터 전체 게임플레이(MainMenu·데모·배송·완료 화면·Pause·설정·오디오·종료)까지 사용자 수동 테스트 승인 완료 — 상세는 `docs/TASKS.md` 섹션 35 참고
- Performance & Compatibility(T084) — 헤드리스 환경은 `DisplayServer.window_set_size()`가 사실상 무동작이고 Stretch(`canvas_items`+`expand`) 재계산에 필요한 실제 창이 없어, 해상도별 UI 잘림·Crosshair 중앙 유지 자동 검증은 신뢰할 수 없는 것으로 판명되어 자동 검증 결과에서 제외했다(`docs/TECH_DEBT.md` TD-021 참고) — 900×500 등 해상도 최종 확인은 사용자 수동 테스트 필요
- Environment Art, Lighting, Visual Target Pass(T085B) — 헤드리스 환경은 렌더링을 하지 않아(더미 드라이버) 실제 명암·Bloom·안개 강도·Crosshair/Package 가독성·"Steam 스크린샷 수준" 달성 여부를 전혀 검증할 수 없었다 — 전적으로 사용자 수동 테스트 필요. `shadow_enabled`/`ssao_enabled` 활성화의 실제 프레임 영향도 헤드리스로 측정 불가(정밀 최적화는 T085C 예정). 외부 Asset(Blender/CC0)은 준비·검수 파이프라인이 없어 이번에도 사용하지 않고 기본 Primitive만 사용했다 — 필요하면 후속 단계에서 재검토 가능
- (구) Art Direction & Delivery Map Blockout / (구) Environment Art, Lighting, Visual Target Pass — `PrototypeLevel.tscn` 기준으로 수행했던 위 T085A/T085B 작업 기록은 `docs/TASKS.md` 섹션 37-ARCHIVE에 보존된다. `PrototypeLevel.tscn` 자체는 회귀 테스트 Scene으로 계속 남아 있으나, 공개 데모 레벨 역할은 신규 `DeliveryDemoLevel.tscn`(T085A 재정의)으로 완전히 대체되었다.
- Delivery Demo Level Design & Greybox(T085A 재정의) — 게이트의 정확한 밀기 저항감·자동 닫힘 속도는 합성 torque 기반 헤드리스 테스트로 정밀 검증이 불가능해(같은 HingeJoint3D가 `apply_torque_impulse()`에는 반응하지만 매 프레임 `apply_torque()`에는 반응이 불안정한 것으로 확인) 구조적 건전성(고착 없음, 열림·버팀 가능)만 자동 검증했다 — 실제 "손맛"은 T085C에서 사용자 피드백 기반으로 튜닝 예정. 에디터 미리보기 없이 좌표 계산만으로 배치해 시각적 비례감·가독성은 사용자 수동 테스트 및 T085B(Environment Art & Lighting)에서 확인·보완 필요. 상세는 `docs/TASKS.md` 섹션 37 "예상 위험" 참고.
