...
## villa-48 — 차량·전체 배송·평가

FullDeliverySession은 OnlineSession.full_route를 활성화한다. 기존 OnlineSession은 빌라 연습을 유지한다. FreightRun이 같은 OnlineLevel 안에 FreightDepot/DeliveryVan을 생성하고 적재→운전→빌라 배송 상태, 마감 시각, 상태/분실, 복구 감점과 결과를 관리한다. DeliveryOrders/DeliveryZone의 목표를 그대로 사용한다. DeliveryVan은 1100kg RigidBody 차체이며 택배는 Gameplay 아래 독립 RigidBody로 남는다. 차량 레이어 8을 전체 배송 플레이어/택배의 충돌 및 잡기 차단에 포함한다. 좌석에 있는 캐릭터는 충돌·보행을 비활성화하고 하차 후 3물리 프레임 동안 충돌을 격리해 이동 경로의 잘못된 충격을 방지한다.

호스트가 차량 힘/좌석/문/파손/시간/평가를 판정하고 클라이언트는 복제한다. 20Hz _state와 _freight_state를 별도 패킷으로 보내 4인 다건의 MTU 초과를 피한다. 종료는 신뢰성 RPC이며 이전 세대/종료 후 차량 패킷을 무시한다. 프로토콜 48은 버전 및 전체 배송/빌라 연습 모드 불일치를 거부한다. 메뉴 또는 입력 500ms 지연은 운전자 입력을 중립으로 만들어 제동한다. 연결 종료 시 기존 대기실 복귀 정책을 유지한다.

Package 손상 기능은 전체 배송에서만 활성화한다. 복구는 상태/실패를 유지하며 순간 이동 중 접촉을 격리한다. 평가 사유/점수/등급은 room_record의 최근 주문에 함께 저장한다. 영구 경제/저장은 없다. 제한 시간·점수 식과 최종 검증은 WINDOWS_PLAYTEST_48.md 참조.


## villa-47 — 4인 온라인 세션 구조

OnlineSession.peer_slots는 [호스트 ID, 참가자 ID 또는 0 ×3]의 고정 슬롯 표다. RPC 발신 ID로 슬롯을 조회해 해당 입력/선택/복구/요청만 처리하며 입력 시각과 준비 상태도 슬롯별로 유지한다. OnlineLevel.couriers는 P1~P4를 제공하고 빈 슬롯의 화면·물리·운반 충돌을 비활성화한다. 호스트는 참가자 전원의 로드 완료 후 시뮬레이션을 시작하고 20Hz 스냅샷을 전송한다. 참가자는 자기 카메라와 호스트 복제 상태를 사용한다. 준비/종료 합의는 현재 점유 슬롯만 집계한다.

온라인 퇴장: 배송 중이면 남은 전원이 현재 주문의 대기실로 복귀, 리스너와 완료한 이전 코스 실적 유지. 빈 슬롯 재참가 가능. 배송 중 새 참가는 사유를 표시하고 거절한다. 호스트 이전/자동 중계는 없다. 프로토콜 47.

GrabbableBody의 Player 차단 경로는 초기 잡기 감지와 같은 CameraPivot→택배 중심을 사용한다. 물리 힘의 목표인 HoldPoint가 벽 너머로 회전하거나 표면 부착점이 계단에 눌려도 실제 택배가 보이면 유지한다. 실제 몸과 택배 중심 사이의 벽은 차단한다. 기존 6cm 접촉 보정은 그대로 유지하며 허용 수치를 키우는 변경은 채택하지 않았다. 힘·무게·충돌 자체는 유지한다. WINDOWS_PLAYTEST_47.md의 재현과 회귀 검사 참고.
