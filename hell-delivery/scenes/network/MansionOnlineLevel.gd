...
# "대저택 정원 미로" 배송 노선의 온라인 레벨 스크립트. ApartmentOnlineLevel.gd와 완전히 같은
# 역할(최대 4인 스폰, 온라인 세션이 기대하는 범용 인터페이스 제공)을 하는 단일 목적지 레벨이라
# 구조를 그대로 따른다 — 빌라 전용 다중 목적지 로직이 없어 PrototypeLevel.gd를 직접 확장한다.
var order_id := "mansion"
...
func _ready() -> void:
	var garden: MansionGarden = $Environment/Garden
	delivery_zone.destination_id = DeliveryOrders.MANSION_DOOR
	delivery_zone.display_name = "대저택 현관"
	delivery_zone.destination_name = "대저택 정원 안쪽 현관"
	delivery_zone.target_package_count = 1
	delivery_zone.global_position = garden.door_position
	destinations = [delivery_zone]
	var package: Package = $Gameplay/Package
	package.configure_destination(DeliveryOrders.MANSION_DOOR, "대저택 현관")
...
func _update_stops() -> void:
	...
	var status: String = "현관 완료" if delivery_zone.delivered_count > 0 else "현관 배송 대기"
